...
QtSpeech *QtSpeech::speech = NULL;

QtSpeech* QtSpeech::instance()
{
    if (speech == NULL) {
        speech = new QtSpeech;
    }
...
QtSpeech::QtSpeech(VoiceInfo n, QObject * parent)
    :QObject(parent), d(new Private)
{
    ULONG count = 0;
...
const VoiceInfo & QtSpeech::name() const {
    return d->name;
}

QList<VoiceInfo> QtSpeech::voices()
{
    QList<VoiceInfo> vs;       
    ULONG count = 0;
    CComPtr<IEnumSpObjectTokens> voices;
...
void QtSpeech::setVoice(VoiceInfo v)
{
    ULONG count = 0;
    CComPtr<IEnumSpObjectTokens> voices;
    ...
    for(int i=0; i< count; i++) {
        ...
        QString id = QString::fromWCharArray(w_id);
        QString name = QString::fromWCharArray(w_name);
        if (id==v.id && name==v.name) {
            d->voice->SetVoice(voice);
            voice.Release();
            break;
        }        
        voice.Release();
...
VoiceInfo QtSpeech::getDefaultVoice()
{
    VoiceInfo VoiceInfo;
    WCHAR * w_id = 0L;
    WCHAR * w_name = 0L;
    ...
    SysCall( SpGetDescription(voice, &w_name), InitError);
    SysCall( voice->GetId(&w_id), InitError);
    VoiceInfo.name = QString::fromWCharArray(w_name);
    VoiceInfo.id = QString::fromWCharArray(w_id);
    voice.Release();
    return VoiceInfo;
}

QtSpeech::State QtSpeech::getStatus()
{
    return d->state;
}
...
void QtSpeech::stop()
{
    Private::WCHAR_Holder w_text("");    
    SysCall(d->voice->Speak(w_text.w, SPF_PURGEBEFORESPEAK, 0), LogicError);
}

QList<VoiceInfo> QtSpeech::getEnglishVoices()
{
    QList<VoiceInfo> vs;       
    ULONG count = 0;
    CComPtr<IEnumSpObjectTokens> voices;
    ...
    CoInitialize(NULL);
    SysCall( SpEnumTokens(SPCAT_VOICES, L"vendor=microsoft;language=409;", NULL, &voices), LogicError);
    SysCall( voices->GetCount(&count), LogicError);
...
QList<VoiceInfo> QtSpeech::getChineseVoices()
{
    QList<VoiceInfo> vs;       
    ULONG count = 0;
    CComPtr<IEnumSpObjectTokens> voices;
    ...
    CoInitialize(NULL);
    SysCall( SpEnumTokens(SPCAT_VOICES, L"vendor=microsoft;language=804;", NULL, &voices), LogicError);
    SysCall( voices->GetCount(&count), LogicError);
